...
const ProductDetail = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            ...
            if (error) {
                setError(t('fetchError')); // Çeviriyi kullan
            } else {
                setProduct(data);
            }
            setLoading(false);
    ...
    const toggleWishlist = async () => {
        ...
        if (error) {
            ...
            alert(newStatus === 'liked' ? t('addedToWishlist') : t('removedFromWishlist')); // Çeviriyi kullan
        }
    };

    if (loading) {
        return <div className="loading-spinner">{t('loading')}</div>; // Çeviriyi kullan
    }
    ...
    if (!product?.product_name) {
        return <div>{t('productNotFound')}</div>; // Çeviriyi kullan
    }
        ...
        <>
            ...
            <div className="product-detail">
                ...
                {product.discount_price > 0 && (
                    <>
                        <p className="discount-price" style={{ textDecoration: 'line-through' }}>
                ...
                >
                    {product.product_wishlist === 'liked' ? t('removeFromWishlist') : t('addToWishlist')} {/* Çeviriyi kullan */}
                </button>
            </div>
        </>
    );
